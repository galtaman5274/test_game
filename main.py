...
SPRITE_SHEET = pygame.image.load('images/roguelikeChar_transparent.png')
# Load background music
music.play('background_music')
music.set_volume(0.5)  # Set the volume level
...
# Game Over buttons
GO_MENU_BUTTON = Rect((WIDTH // 2 - 100, HEIGHT // 2 + 20), (200, 50))
EXIT_BUTTON_GAMEOVER = Rect((WIDTH // 2 - 100, HEIGHT // 2 + 90), (200, 50))
...
class Hero:
    """Hero class to encapsulate the hero behavior and animation."""

    def __init__(self, frames, start_pos, speed):
        self.frames = frames
        self.current_frame = 0
        self.animation_speed = 0.2  # Time delay between frames
        self.animation_timer = 0
        self.pos = list(start_pos)
        self.speed = speed

    def update(self, dt):
        """Update the hero's position, animation, and check for movement."""
        moving = False
        if keyboard.left:
    ...
    def draw(self):
        """Draw the hero using the current frame."""
        screen.blit(self.frames[self.current_frame], self.pos)

    def get_rect(self):
        """Returns the hero's rectangular collision box."""
        return Rect(self.pos[0], self.pos[1], TILE_SIZE, TILE_SIZE)
...
# Game state
hero = Hero(frames, (WIDTH // 4, HEIGHT // 4), 3)
...
def reset_game():
    ...
    hero.pos = [WIDTH // 3, HEIGHT // 3]  # Reset hero position
...
def draw_gameplay():
    ...
    if game_over:
        ...
        screen.draw.filled_rect(GO_MENU_BUTTON, "green")
        screen.draw.text("Main Menu", center=GO_MENU_BUTTON.center, fontsize=40, color="white")
        screen.draw.filled_rect(EXIT_BUTTON_GAMEOVER, "red")
        screen.draw.text("Exit", center=EXIT_BUTTON_GAMEOVER.center, fontsize=40, color="white")
    else:
        hero.draw()
...
def on_mouse_down(pos):
    ...
    if not game_active:
        ...
        if GO_MENU_BUTTON.collidepoint(pos):
            game_active = False
            music.stop()
        elif EXIT_BUTTON_GAMEOVER.collidepoint(pos):
            sys.exit()
